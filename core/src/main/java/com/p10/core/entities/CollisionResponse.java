...
//        float pushStrength = 8.0f;// this one can play around ngl to fit the code. (will try to find another
        ...
        float pushStrength1 = e1.getMass() / totalMass;
        float pushStrength2 = e2.getMass() / totalMass;
        if (dist > 0) { // fixed this as it was inverted . the bug crashed the game. when this runs with
            ...
            if (!e1.isKinematic()) {
            	e1.getPosition().x += relativeSpeed * pushStrength1;
                e1.getPosition().y += relativeSpeed * pushStrength1;
                
                
            }
            if (!e2.isKinematic()) {
                e2.getPosition().x -= relativeSpeed * pushStrength2;
                e2.getPosition().y -= relativeSpeed * pushStrength2;
            }
        }
    }
}
